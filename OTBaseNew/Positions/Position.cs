...
namespace OTBaseNew.Positions
{
    ...
    public class Position
    {
        ...
        public void Save()
        {
            ...
            {
                //Строка-запрос
                query = string.Format("INSERT INTO `Positions`(`name`, `created`) VALUES ('{0}','{1}'); SELECT * FROM `Positions` order by id desc;", Name, MySqlWorker.DataBase.ConvertDateToMySqlString(Created));
                //Создает запрос и возвращает результат
